...
- API tests cover `/health`, subject-hash salt health reporting, `/openapi.json` privacy fields plus Wavy no-result and timeout responses, a Bankaool score response, invalid institution rejection in mock mode, the live Wavy Node adapter request shape, and gateway-timeout handling for stalled Wavy requests.
