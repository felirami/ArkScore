...
`pnpm submission:evidence:write` writes a non-secret `docs/SUBMISSION_EVIDENCE.md` packet with the current commit, public deployment targets from the configured environment or Fuji deployment artifact, the latest Fuji score-record proof when `LatestScoreRecord.json` exists, hosted smoke, live verifier, requirements audit, judge demo runbook, readiness output, and final handoff commands. The generated handoff commands include `pnpm plan:eerc20` before the optional eERC20 probe, export the Railway API URL plus Fuji registry/scorer addresses once for the remaining final steps, and run `pnpm readiness:strict:record` after `pnpm record:fuji`; when `ARKSCORE_REQUIRE_EERC20=true`, they also export `ARKSCORE_REQUIRE_EERC20=true`, use `pnpm probe:eerc20:strict`, and finish with `pnpm verify:live:strict:eerc20:record`.
