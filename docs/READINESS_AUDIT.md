...
`pnpm readiness` produces a non-secret live-gate report covering Vercel reachability, Vercel CLI auth, Railway auth, Wavy credentials, Fuji deployer configuration, frontend deployment variables, and the latest Fuji score-record artifact when configured. It accepts `ARKSCORE_WEB_URL` for the submitted frontend URL plus the same Railway API, Fuji registry, scorer, and score-record artifact aliases used by the finalization and Hardhat scripts, so the report mirrors the actual handoff flow. The API URL gate requires a public HTTPS Railway URL, preventing copied local values such as `http://localhost:4000` from satisfying final readiness. `--skip-cli-auth` skips only the Railway/Vercel auth probes while still checking the configured web URL for deterministic local tests. `pnpm readiness:strict:record` makes `LatestScoreRecord.json` mandatory after `pnpm record:fuji` and requires that artifact to be Wavy-backed, even if `ARKSCORE_ALLOW_MOCK_RECORD=true` was used for a temporary demo.
