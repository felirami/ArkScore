...
- `pnpm record:fuji` gives judges a CLI-verifiable proof that a live Wavy-backed Railway score had its evidence hash recomputed, was written by the configured authorized scorer, read back through `CreditScoreRegistry`, and captured as a non-secret `LatestScoreRecord.json` artifact for the submission evidence packet.
