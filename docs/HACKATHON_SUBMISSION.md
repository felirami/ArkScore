...
- `pnpm verify:railway:live` gives the team a post-deployment proof that the Railway API itself is serving live Wavy-backed health, OpenAPI, fresh/evidence-hashed score, no-store cache, and rate-limit responses before Fuji/Vercel finalization.
...
- `pnpm record:fuji` gives judges a CLI-verifiable proof that a fresh live Wavy-backed Railway score had its evidence hash recomputed, was written by the configured authorized scorer, read back through `CreditScoreRegistry`, and captured as a non-secret `LatestScoreRecord.json` artifact for the submission evidence packet.
