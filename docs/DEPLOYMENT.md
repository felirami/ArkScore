...
`verify:railway:live` is the API-only strict live gate. It skips the Vercel bundle and Fuji contract checks, then verifies Railway `/health`, production subject-hash salt mode, live Wavy mode, `/openapi.json` with the served API origin in `servers`, score response shape, recomputed score evidence hash, no-store score cache headers, and score rate-limit headers.
