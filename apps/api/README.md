...
The API calls Wavy Node when `WAVY_NODE_API_KEY` and `WAVY_NODE_PROJECT_ID` are configured. Otherwise `WAVY_NODE_MOCK_MODE=auto` returns deterministic demo data with the same response shape. The Wavy adapter also exposes a supported-chain helper used by `pnpm probe:wavy` to verify that the configured `WAVY_NODE_CHAIN_ID` is active before the live score call. Live Wavy requests use `WAVY_NODE_TIMEOUT_MS` so Railway returns a clear gateway timeout if the upstream stalls. Score requests are rate-limited per client with `ARKSCORE_SCORE_RATE_LIMIT_MAX` and `ARKSCORE_SCORE_RATE_LIMIT_WINDOW_MS`.
